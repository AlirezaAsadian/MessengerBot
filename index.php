   ...
   $json=json_decode(file_get_contents("dasturat.json"),true);
    ...
    file_put_contents("bakhsh.txt","empty");
      var_dump(bot("sendmessage",[
          ...
          "parse_mode"=>"markdown",
          "reply_markup"=>json_encode(["remove_keyboard"=>true,"inline_keyboard"=>[[["text"=>"آمار 👥","callback_data"=>"amar"],],[["text"=>"فروارد همگانی 🗣","callback_data"=>"forward"],["text"=>"بلاک لیست 🚫","callback_data"=>"block"]],[["text"=>"♨️ پیام استارت ربات ♨️","callback_data"=>"start"]],[["text"=>"✏️فروارد بدون عنوان✏️","callback_data"=>"forward2"]],[["text"=>"دستور ➕","callback_data"=>"dasturjadid"],["text"=>"دستور ➖","callback_data"=>"hazfdastur"]],[["text"=>"منو ➕","callback_data"=>"createmenu"],["text"=>"منو ➖","callback_data"=>"deletemenu"]],[["text"=>"⚜ مدیریت منو ⚜","callback_data"=>"managementmenu"]]]])
        ]));
    }
  }else{
